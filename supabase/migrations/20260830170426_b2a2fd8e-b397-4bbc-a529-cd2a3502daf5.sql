...
DECLARE
  ...
  n int := 1;
BEGIN
  -- Authorization: either a signed-in admin (direct call) or the trusted
  -- service role (server function, which verifies the admin role itself).
  IF auth.uid() IS NOT NULL THEN
    IF NOT public.has_role(auth.uid(), 'admin'::app_role) THEN
      RAISE EXCEPTION 'Only admins can duplicate pieces' USING ERRCODE = 'insufficient_privilege';
    END IF;
  ELSIF current_user <> 'service_role' AND NOT pg_has_role(current_user, 'service_role', 'MEMBER') THEN
    RAISE EXCEPTION 'Only admins can duplicate pieces' USING ERRCODE = 'insufficient_privilege';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.duplicate_product(uuid) FROM PUBLIC, anon, authenticated;
